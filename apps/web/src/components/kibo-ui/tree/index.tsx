...
import { ChevronRight, File, Folder, FolderOpen } from "@/components/iconimate";
...
export const TreeProvider = ({
  ...
  const currentExpandedIds = isExpandedControlled
  ...
  const toggleExpanded = useCallback(
    ...
    [currentExpandedIds, isExpandedControlled, onExpandedChange]
  );
  ...
  return (
    <TreeContext.Provider
      value={{
        ...
        multiSelect,
        indent,
        animateExpand: animateExpand && !prefersReducedMotion,
      }}
    >
      <motion.div
...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        "group relative mx-1 flex cursor-pointer items-center rounded-md px-3 py-2 transition-colors duration-150",
        "hover:bg-accent/50",
...
export const TreeExpander = ({
  ...
  const { animateExpand, expandedIds, toggleExpanded } = useTree();
  const { nodeId } = useTreeNode();
  const isExpanded = expandedIds.has(nodeId);
...
export const TreeIcon = ({
  ...
  const { animateExpand, showIcons, expandedIds } = useTree();
  const { nodeId } = useTreeNode();
  const isExpanded = expandedIds.has(nodeId);
